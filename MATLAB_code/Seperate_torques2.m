function [coefficients, unknowns] = Seperate_torques2()
...
unknown = x;
modify_unknowns = x;
for i = 1:length(singular_unknowns)
    modify_unknowns(i) = 0;
end
cof = torques;
for k = 1:7
                ...
                if isempty(symvar(subs(cof(2), [q1, q2, q3, q4, q5, q6, q7], [pi/3 pi/3 pi/3 pi/3 pi/3 pi/3 pi/3])))
                    for w = 1:(unknowns_count+1)
                        if w == (unknowns_count+1)
                            %adds unknown
                            for m = 1:(unknowns_count+1)
                                if m == (unknowns_count+1)
                                    unknowns_count = unknowns_count + 1;
                                    coefficients(k,unknowns_count) = cof(2);
                                    unknowns(unknowns_count) = unknown;
                                elseif cof(2) == coefficients(k, m)
                                    if length(coeffs(modify_unknowns(m),unknown))==2
                                        break
                                    else
                                        modify_unknowns(m) = modify_unknowns(m) + unknown;
                                        break;
                                    end
                                end
                            end
...
unknowns = unknowns + modify_unknowns(1:length(unknowns));