...
#pragma once
#ifndef _sawConstraintController_mtsVFDataCartesianTranslation_h
#define _sawConstraintController_mtsVFDataCartesianTranslation_h
...
/* source line: 7 */
class CISST_EXPORT mtsVFDataCartesian: public mtsVFDataBase
{
 /* default constructors and destructors. */
 public:
    mtsVFDataCartesian(void);
    mtsVFDataCartesian(const mtsVFDataCartesian & other);
    ~mtsVFDataCartesian();

/* source line: 18 */
 public:
    vct3 OffsetVector; // OffsetVector
    /* default methods */
 public:
 ...
 public:
    void Copy(const mtsVFDataCartesian & source);
...
// mts-proxy set to true
typedef mtsGenericObjectProxy<mtsVFDataCartesian> mtsVFDataCartesianProxy;
CMN_DECLARE_SERVICES_INSTANTIATION(mtsVFDataCartesianProxy);

/* default functions */
void CISST_EXPORT cmnSerializeRaw(std::ostream & outputStream, const mtsVFDataCartesian & object);
void CISST_EXPORT cmnDeSerializeRaw(std::istream & inputStream, mtsVFDataCartesian & placeHolder);
/* data functions */
template <> class cmnData<mtsVFDataCartesian > {
public: 
    enum {IS_SPECIALIZED = 1};
    typedef mtsVFDataCartesian DataType;
...
inline std::ostream & operator << (std::ostream & outputStream, const mtsVFDataCartesian & data) {
    outputStream << cmnData<mtsVFDataCartesian >::HumanReadable(data);
    return outputStream;
}
#if CISST_HAS_JSON
template <> void CISST_EXPORT cmnDataJSON<mtsVFDataCartesian >::SerializeText(const mtsVFDataCartesian & data, Json::Value & jsonValue);
template <> void CISST_EXPORT cmnDataJSON<mtsVFDataCartesian >::DeSerializeText(mtsVFDataCartesian & data, const Json::Value & jsonValue) throw (std::runtime_error);
#endif // CISST_HAS_JSON


#endif // _sawConstraintController_mtsVFDataCartesianTranslation_h
